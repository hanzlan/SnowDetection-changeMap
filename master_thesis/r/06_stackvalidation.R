...
for( i in 1:length(doy_cos)) {
  ...
  writeRaster(stk, filename=file.path(paste0("validation_stacks/aqua/predstack_", substr(doy_cos[i], 9, 16), ".tif")),
...
for( i in 1:length(doy_cos)) {
  ...
  stk <- stack(doycos_rf, radterra_rf, lagradterra_rf, dem_rf, minuteterra_rf,
               year_rf, lat_rf, lon_rf)
  writeRaster(stk, filename=file.path(paste0("validation_stacks/terra/predstack_", substr(doy_cos[i], 9, 16), ".tif")),
              format="GTiff", overwrite=TRUE)
}
